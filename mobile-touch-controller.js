...
#obiTouchPad{position:fixed!important;inset:0!important;width:100vw!important;height:100vh!important;z-index:2147483000!important;pointer-events:none!important;font-family:Arial,sans-serif!important;display:none!important}
#obiTouchPad.on{display:block!important}
#obiTouchPad .obiJoy{position:absolute;left:max(14px,env(safe-area-inset-left));bottom:max(14px,env(safe-area-inset-bottom));width:126px;height:126px;border-radius:50%;background:rgba(5,8,12,.72);border:2px solid rgba(255,255,255,.42);box-shadow:inset 0 0 25px rgba(255,255,255,.06),0 8px 22px rgba(0,0,0,.55);pointer-events:auto!important;touch-action:none!important}
...
#obiTouchPad button{position:absolute!important;width:62px;height:62px;border-radius:50%;background:rgba(5,8,12,.78);border:2px solid rgba(255,255,255,.35);color:#fff;font-weight:1000;font-size:18px;box-shadow:0 7px 18px rgba(0,0,0,.5);pointer-events:auto!important;touch-action:none!important;-webkit-user-select:none!important}
#obiTouchPad button:active,#obiTouchPad button.hit{transform:scale(.9)!important}
#obiTouchPad .bT{right:71px;top:0}.bO{right:8px;top:71px;background:rgba(215,25,32,.88)!important}.bX{left:71px;top:71px}.bQ{left:8px;top:71px}
...
function sync(){addStyle();const r=root();bind(r);r.classList.toggle('on',gameVisible())}
