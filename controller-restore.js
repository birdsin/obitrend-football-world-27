...
#vL{
 ...
 background:#07100dcc!important;border:2px solid #ffffff38!important;
 box-shadow:inset 0 0 25px #0008!important;pointer-events:auto!important;touch-action:none!important;
}
...
#vR{position:absolute!important;right:12px!important;bottom:14px!important;width:185px!important;height:185px!important;pointer-events:auto!important;touch-action:none!important}
...
#vOpt{position:absolute!important;right:16px!important;top:14px!important;width:68px!important;height:35px!important;border-radius:10px!important;background:#07100ddd!important;color:#fff!important;font-size:8px!important;font-weight:1000!important;pointer-events:auto!important}
@media(max-width:600px){
 #vL{width:120px!important;height:120px!important}
 #vR{width:185px!important;height:185px!important}
}
`;
}
...
function createFallback(){
 ...
 function press(id,down,up){
  const b=r.querySelector(id);if(!b)return;
  b.addEventListener('pointerdown',e=>{e.preventDefault();b.classList.add('h');try{down&&down()}catch(err){console.error('[OBI CONTROLLER]',err)}},{passive:false});
  ['pointerup','pointercancel'].forEach(t=>b.addEventListener(t,e=>{e.preventDefault();b.classList.remove('h');try{up&&up()}catch(err){console.error('[OBI CONTROLLER]',err)}},{passive:false}));
 }
...
function sync(){
 ...
 r.style.setProperty('pointer-events',on?'none':'none','important');
}
