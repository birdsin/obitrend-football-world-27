...
{name:'Kylian Mbappé',club:'Real Madrid',nation:'France',pos:'ST',ovr:91,pac:96,sho:91,pas:80,dri:92,def:29,phy:76}
];
...
function apply(){
 ...
 if(!document.getElementById('obiRealPlayerStyle')){const s=document.createElement('style');s.id='obiRealPlayerStyle';s.textContent='.obiFifaPlayer.realPlayer .name{font-size:7px;opacity:1;letter-spacing:.1px}.obiFifaPlayer.realPlayer .kit{box-shadow:0 3px 6px #0008}';document.head.appendChild(s)}
 return true;
}
function showCard(){
 const game=document.getElementById('game');if(!game)return;
 let card=document.getElementById('obiRealCard');if(!card){card=document.createElement('div');card.id='obiRealCard';card.className='obiRealCard';game.appendChild(card)}
 const active=[...document.querySelectorAll('.obiFifaPlayer')].find(x=>x.classList.contains('controlled'));
 if(!active){card.classList.remove('show');return}
 card.innerHTML='<b>'+active.dataset.player+'</b><span>'+active.dataset.nation+' • '+active.dataset.club+' • '+active.dataset.playerPos+'</span><br><em>OVR '+active.dataset.ovr+' • PAC '+active.dataset.pace+' • SHO '+active.dataset.shooting+' • PAS '+active.dataset.passing+' • DRI '+active.dataset.dribbling+'</em>';card.classList.add('show');
}
function watch(){apply();new MutationObserver(()=>{if(apply())showCard()}).observe(document.body,{childList:true,subtree:true});setInterval(()=>{if(apply())showCard()},1000)}
window.OBITREND_REAL_PLAYERS=players;
window.showRealPlayerCard=showCard;
window.OBITREND_ACTIVE_PLAYER=()=>{const e=[...document.querySelectorAll('.obiFifaPlayer')].find(x=>x.classList.contains('controlled'));return e?players.find(p=>p.name===e.dataset.player)||null:null};
