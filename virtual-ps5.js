...
`;
  ...
  function build(){
    ...
    root.innerHTML=`<div id="vps5WorldPlayer"></div>
    ...
    document.body.appendChild(root);bindLeft();bindButtons();bindRight();
  }
  ...
  function move(dx,dy){
    ...
    if(sc==="game"){
      ...
      const k=s.sprinting?1.15:.55;s.x=Math.max(10,Math.min(90,Number(s.x||50)+dx*k));s.y=Math.max(10,Math.min(90,Number(s.y||50)+dy*k));s.moving=Math.hypot(dx,dy)>.02;
      if(typeof window.updatePlayer==="function")window.updatePlayer();
    }else if(sc==="world"){
      const p=$("vps5WorldPlayer");if(!p)return;let x=Number(p.dataset.x||50),y=Number(p.dataset.y||70);x=Math.max(4,Math.min(96,x+dx*1.3));y=Math.max(18,Math.min(82,y+dy*1.3));p.dataset.x=x;p.dataset.y=y;p.style.left=x+"%";p.style.top=y+"%";
    }
  }

  function bindLeft(){
    const base=$("vps5Left"),stick=$("vps5Stick");if(!base)return;
    const update=e=>{const r=base.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,max=r.width*.32;let x=e.clientX-cx,y=e.clientY-cy,d=Math.hypot(x,y);if(d>max){x=x/d*max;y=y/d*max}stick.style.transform=`translate(calc(-50% + ${x}px),calc(-50% + ${y}px))`;move(x/max,y/max)};
  ...
  function bind(id,down,up){const el=$(id);if(!el)return;const press=e=>{e.preventDefault();el.classList.add("held");if(down)down()};const release=e=>{e.preventDefault();el.classList.remove("held");if(up)up()};el.addEventListener("pointerdown",press,{passive:false});el.addEventListener("pointerup",release);el.addEventListener("pointercancel",release);el.addEventListener("pointerleave",release)}
  const call=n=>typeof window[n]==="function"?window[n]():null;
  function through(){const s=stateRef();if(!s||s.paused||screen()!=="game")return;s.ballX=Math.min(96,Number(s.x||50)+17);s.ballY=Number(s.y||50);call("updateBall");call("commentary")||0;if(typeof window.recordReplayEvent==="function")window.recordReplayEvent("THROUGH_BALL");if(typeof window.showMessage==="function")window.showMessage("△ THROUGH BALL")}
  function tackle(){if(screen()!=="game")return;if(typeof window.recordReplayEvent==="function")window.recordReplayEvent("TACKLE");if(typeof window.commentary==="function")window.commentary("Strong defensive pressure.");if(typeof window.showMessage==="function")window.showMessage("□ TACKLE / PRESS")}
  function switchPlayer(){if(screen()!=="game")return;if(typeof window.recordReplayEvent==="function")window.recordReplayEvent("PLAYER_SWITCH");if(typeof window.showMessage==="function")window.showMessage("L1 PLAYER SWITCH")}
  function skill(){const s=stateRef();if(!s||s.paused||screen()!=="game")return;if(typeof window.recordReplayEvent==="function")window.recordReplayEvent("SKILL");if(typeof window.commentary==="function")window.commentary("A quick skill move.");if(typeof window.showMessage==="function")window.showMessage("R1 SKILL MOVE")}
  function protect(on){const s=stateRef();if(screen()!=="game")return;if(s)s.protecting=!!on;if(on!==lastProtect&&typeof window.showMessage==="function")window.showMessage(on?"L2 PROTECT / AIM":"L2 RELEASE");lastProtect=on}
  ...
  function bindButtons(){
    ...
    bind("vR2",()=>sprint(true),()=>sprint(false));bind("vL2",()=>protect(true),()=>protect(false));bind("vL1",switchPlayer);bind("vR1",skill);bind("vOptions",options);
  }
  ...
  function bindRight(){
    ...
    const end=e=>{if(rightId===e.pointerId){rightId=null;window.vps5Camera={x:0,y:0}}};area.addEventListener("pointerup",end);area.addEventListener("pointercancel",end);
  }
  ...
  function physicalLoop(){
    ...
    if(p){if(s)s.gamepad=p;
      if(screen()==="game"&&!s?.paused){if(edge(p,2))tackle();if(edge(p,3))through();if(edge(p,4))switchPlayer();if(edge(p,5))skill();protect(pressed(p,6))}
      dpad(p);physicalCamera(p);prev=(p.buttons||[]).map(b=>!!(b&&(b.pressed||Number(b.value||0)>.55)));
    }else{padIndex=null;prev=[];lastDpad=0;if(s&&s.gamepad){s.gamepad=null;s.sprinting=false;s.moving=false}}
    requestAnimationFrame(physicalLoop);
  }

  window.addEventListener("gamepadconnected",e=>{padIndex=e.gamepad?.index??null;const s=stateRef();if(s)s.gamepad=e.gamepad;const st=$("controllerStatus");if(st)st.textContent="Connected: "+e.gamepad.id;if(typeof window.setControlMode==="function"&&screen()==="game"){window.setControlMode("gamepad");if(typeof window.closeController==="function")window.closeController();if(typeof window.showMessage==="function")window.showMessage("🎮 DUALSENSE CONNECTED")}});
  window.addEventListener("gamepaddisconnected",e=>{if(padIndex===e.gamepad?.index)padIndex=null;const s=stateRef();if(s&&s.gamepad?.index===e.gamepad.index){s.gamepad=null;s.sprinting=false;s.moving=false}const st=$("controllerStatus");if(st)st.textContent="Controller disconnected.";if(s?.controlMode==="gamepad"&&typeof window.setControlMode==="function")window.setControlMode("touch")});

  function init(){build();const p=getPad(),s=stateRef();if(p&&s)s.gamepad=p;const st=$("controllerStatus");if(st&&p)st.textContent="Connected: "+p.id;setInterval(refresh,120);refresh();requestAnimationFrame(physicalLoop)}
