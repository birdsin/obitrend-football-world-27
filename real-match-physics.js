...
const active=()=>{try{const f=window.OBITREND_PLAYER_PROFILE;if(typeof f==='function')return f();return f||window.OBITREND_ACTIVE_PLAYER||null}catch(e){return null}};
...
function restore(old){const s=getState();if(!s||!old)return;s.lastMoveX=old.x;s.lastMoveY=old.y}
function wrap(name,kind){
 const old=window[name];if(typeof old!=='function'||old.__obiPhysicsWrapped)return;
 const fn=function(){const saved=prepare(kind);try{return old.apply(this,arguments)}finally{restore(saved)}};fn.__obiPhysicsWrapped=true;window[name]=fn;
}
function applySkillBurst(){
 const s=getState(),p=profile();if(!s||!p)return;
 const burst=clamp((.75+p.dribbling/99*.55)*quality('skill'),.72,1.35),x=Number(s.lastMoveX)||0,y=Number(s.lastMoveY)||0,len=Math.hypot(x,y)||1;
 if(Math.hypot(x,y)>.08){s.x=clamp((+s.x||50)+(x/len)*1.2*(burst-.7),5,95);s.y=clamp((+s.y||50)+(y/len)*1.2*(burst-.7),5,95)}
}
function wrapSkill(){
...
function tick(){
 ...
 if(s&&p){
  const stamina=clamp(Number(s.obiMatchStaminaValue??p.stamina??100),0,100),fatigue=stamina<20?.80:stamina<40?.89:stamina<60?.95:1,pace=.72+(p.pace/99)*.38;
  if(s.moving)s.speed=clamp((s.sprinting?.105:.058)*pace*fatigue,.032,.12);
 }
 requestAnimationFrame(tick);
}
function init(){
 wrap('passBall','pass');wrap('throughBall','through');wrap('shootBall','shoot');wrap('tackle','tackle');wrapSkill();
 window.OBITREND_REAL_MATCH_PHYSICS={profile,prepare,quality};requestAnimationFrame(tick);
}
